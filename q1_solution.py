...
def kl_gaussian_gaussian_analytic(mu_q, logvar_q, mu_p, logvar_p):
    ...
    *** note. ***
    ...
    d = mu_q.size(1)
    D = torch.tensor(np.ones((batch_size, d), dtype=float))
...
def kl_gaussian_gaussian_mc(mu_q, logvar_q, mu_p, logvar_p, num_samples=1):
    ...
    *** note. ***
    ...
    # kld
    p = torch.distributions.normal.Normal(loc=mu_p, scale=torch.sqrt(torch.exp(logvar_p)))
    q = torch.distributions.normal.Normal(loc=mu_q, scale=torch.sqrt(torch.exp(logvar_q)))
